...
SpotifyApp::SpotifyApp(SemaphoreHandle_t mutex) : App(mutex)
{
...
{
    EntityStateUpdate new_state_update = updateStateFromKnob(state.motor_state);
...
{
    motor_notifier->requestUpdate(getMotorConfig());
}
...
int8_t SpotifyApp::navigationNext()
{
